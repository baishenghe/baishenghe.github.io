---
permalink: /
title: ""
excerpt: ""
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

{% if site.google_scholar_stats_use_cdn %}
{% assign gsDataBaseUrl = "https://cdn.jsdelivr.net/gh/" | append: site.repository | append: "@" %}
{% else %}
{% assign gsDataBaseUrl = "https://raw.githubusercontent.com/" | append: site.repository | append: "/" %}
{% endif %}
{% assign url = gsDataBaseUrl | append: "google-scholar-stats/gs_data_shieldsio.json" %}

<span class='anchor' id='about-me'></span>

I am **Lin Liu (刘琳)**, currently a Master's student at **Peking University** (enrolled in 2026), advised by Prof. Xinyu Liu. Prior to that, I received my Bachelor's degree in Engineering from the **School of Computer Science and School of Software, Nanjing University of Information Science and Technology (NUIST)** from 2022 to 2026, where I was advised by Prof. Wenjie Liu.

My research focuses on the application of artificial intelligence in healthcare. Specifically, I am interested in **Medical Large Language Models**—exploring how LLMs can support clinical question answering, medical reasoning, and diagnostic assistance; **Embodied AI**—studying how intelligent agents perceive and make decisions in the physical world; and **Medical Image Analysis**—developing methods for the analysis, understanding, and computer-aided diagnosis of medical images. Through this work, I hope to bring AI closer to real-world medical needs.

This site is where I document my research and growth. If you are interested in my work or would like to collaborate, please feel free to reach out via email. You can also find my [CV](files/resume.pdf) here.

# 🔥 News
- *2026.06*: &nbsp;🎉🎉 My academic homepage is officially online!

# 📝 Publications 

# 🎖 Honors and Awards
**National-Level Awards**
- *First Prize*, 14th "China Software Cup" National Software Design Competition (Grand Finals)
- *Meritorious Winner (International First Prize)*, 2024 Mathematical Contest in Modeling (MCM/ICM, USA)
- *Second Prize*, 2024 National College Students Innovation and Invention Competition (Grand Finals)
- *National Scholarship*, Ministry of Education
- President's Scholarship

**Honorary Titles**
- "May Fourth" Youth Model (五四青年标兵)
- Outstanding Merit Student (三好学生标兵)
- Nominee, Annual Student Figure Award (年度大学生人物提名奖)
- University "Sports Star" (体育明星)

# 🔬 Research & Projects

# 🏃 Activities & Leadership
- Class Monitor, Hikvision Class 1, Class of 2022 (2022级海康威视1班班长).
- Former President, Student Union of the School of Software (软件学院学生会主席).
- Current Captain, University Track and Field Team (校田径队队长).
- National Level-2 Athlete (100m); broke three long-standing university records and won 8 championships across track and field competitions at various levels.

# 📖 Educations
- *2026.09 - Present*, M.S. Student, **Peking University**, advised by Prof. Xinyu Liu.
- *2022.09 - 2026.06*, B.Eng., **Nanjing University of Information Science and Technology (NUIST)**, School of Computer Science & School of Software, advised by Prof. Wenjie Liu.




